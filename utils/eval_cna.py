...
def load_cna_data(base_path: str, no_cache: bool = False) -> pd.DataFrame:
    """
    ...
    Raises:
        ValueError: If no CNA files are found or if filenames do not match expected pattern.
    """
    base_path = pathlib.Path(base_path)
    if not base_path.is_dir():
        raise ValueError(f"Base path {base_path} is not a directory.")

    cna_files = list(base_path.rglob("cna*.yaml"))
    if not cna_files:
        raise ValueError(f"No CNA files found in {base_path}.")

    df = pd.DataFrame()
    for cna_file in cna_files:
        logger.info(f"Processing {cna_file}...")
        match = re.match(r"cna(\d+)K\.yaml", cna_file.name)
        if not match:
            raise ValueError(
                f"Filename {cna_file.name} does not match expected pattern."
            )
        temperature = float(match.group(1))
        data = load_yaml(cna_file, no_cache=no_cache)

        header = data["keywords"]
        body = data["data"]

        values = pd.DataFrame(body, columns=header)["c_cna"].values.ravel()
        ...
        df_local["phase"] = cna_file.parent.name.upper()
...
def plot_cna(
    ...
    """
    Plot the CNA data from the DataFrame.

    Args:
        df (pd.DataFrame): DataFrame containing the CNA data.
    ...
    if update_layout:
        fig.update_layout(
            title="CNA Phase Fractions vs Temperature",
...
def main(base_path: str, show: bool = False, no_cache: bool = False) -> None:
    """
    Main function to load CNA data and plot it.
    Args:
        base_path (str): Path to the directory containing CNA data files.
        show (bool): Whether to display the plot in a browser.
    """
    cna_df = load_cna_data(base_path, no_cache)

    filename = DATA_DIR / "cna_eval.parquet"
    cna_df.to_parquet(filename, index=False)
    logger.info("CNA results saved to '%s'.", filename)
...
if __name__ == "__main__":
    parser = argparse.ArgumentParser(description="Evaluate CNA data and plot results.")
    parser.add_argument(
        "--base_path",
        type=str,
        help="Path to the directory containing CNA data files.",
        default=None,
    )
    parser.add_argument(
        "--show",
    ...
    main(args.base_path or LAMMPS_OUTPUT_DIR, show=args.show, no_cache=args.no_cache)
